...
export const CursorModeToggle = () => {
    ...
    const [optionElementWidthMap, updateOptionElementWidthMap] =
        useImmer<OptionElementWidthMap>({});
    ...
    const getOptionElementWidthFor = useCallback(
        (mode: CursorMode) => {
     ...
     * @param mode
     */
    const determineOffsetXFor = useCallback(
        (mode: CursorMode) => {
            ...
            return pastOptions.reduce(
                (currentSum, iteratingOption) =>
                    currentSum +
                    getOptionElementWidthFor(iteratingOption.value) +
    ...
    const moveSliderTo = useCallback(
        (mode: CursorMode) => {
            const optionElementWidth = getOptionElementWidthFor(mode);
            const offsetX = determineOffsetXFor(mode);

            sliderBackgroundPositionX.value = withTiming(offsetX, {
                duration: 150,
            });
            selectedOptionElementWidth.value = withSpring(optionElementWidth, {
                duration: 1200,
            });
        },
        [
            determineOffsetXFor,
            getOptionElementWidthFor,
            selectedOptionElementWidth,
            sliderBackgroundPositionX,
        ],
    );
    ...
    const onOptionElementLayoutFor = (
        ...
        updateOptionElementWidthMap((draft) => {
            draft[mode] = width;
        });
    };
    ...
    return (
        <GestureDetector gesture={horizontalSwipeGesture}>
            <View style={styles.container}>
                <Animated.View
                    style={[styles.sliderBackground, animatedStyle]}
                />
                ...
                {OPTIONS.map((option) => (
                    <PressableScale
                        ...
                        style={styles.option}
                        onLayout={(event) =>
                            onOptionElementLayoutFor(event, option.value)
                        }
                    >
                        <View>
                            <Text
                                style={[
                                    styles.optionText,
                                    isCursorModeSelected(option.value)
